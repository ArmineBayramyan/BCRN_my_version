...
def mean_channels(F):
    ...
    Uses .mean() directly for numerical stability.
    """
    assert F.dim() == 4
    return F.mean(dim=(2, 3), keepdim=True)


def stdv_channels(F, eps: float = 1e-6):
    """
    Spatial standard deviation per channel in a numerically stable way.
    F: (N, C, H, W)
    eps: small constant to avoid zero variance / sqrt(0) issues.
    """
    assert F.dim() == 4
    F_mean = mean_channels(F)
    F_centered = F - F_mean

    F_var = (F_centered ** 2).mean(dim=(2, 3), keepdim=True)
    F_var = torch.clamp(F_var, min=eps)

    return torch.sqrt(F_var)
